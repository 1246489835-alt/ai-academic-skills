...
- **Panel labels (a/b/c) are NOT added by default when this skill is invoked in combination with another project/课题 skill** (e.g., mmr-cardiac-injury or any other research project skill). Only add panel labels when: (1) this skill is used standalone without any project skill context, OR (2) the user explicitly requests panel labels. Rationale: final panel numbering is handled during manuscript assembly; hardcoding in scripts creates rework.
